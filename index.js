...
client.on("message", (message) => {
    ...
    if (cmd == `${prefix}안녕`) {
        ...
        if (add == "출석") {
            ...
            return message.channel.send(`${message.author} 이 서버는 ${message.guild.name} 이며, 현재 총 인원수는(봇 포함) ${message.guild.memberCount}명 입니다. 봇을 제외한 인원수는 ${message.guild.members.filter(member => !member.user.bot).size}명 입니다.`);
        } else {
